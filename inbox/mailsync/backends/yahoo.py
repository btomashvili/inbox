...
from inbox.util.misc import cleanup_subject, thread_messages
...
def add_yahoo_attrs(db_session, log, new_uid, flags, folder, created):
    ...
    with db_session.no_autoflush:
        ...
        if (folder.canonical_name in ('drafts', 'sent') and not created
        ...
        if parent_threads == []:
            ...
            blah = thread_messages(parent_thread.messages)
            import pdb ; pdb.set_trace()
            parent_thread.messages = thread_messages(parent_thread.messages)
